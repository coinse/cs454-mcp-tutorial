...
class MCPClient:
    ...
    async def testing_loop(self, target_website):
        ...
        print(f"Testing Agent for the websites: {target_website}")
        self.messages = [{
            "role": "system",
            "content": "You are a testing agent that can interact with web pages and perform tasks."
        }]

        if "https://" not in target_website and "http://" not in target_website:
        ...
        while True:
            ...
            while True:
                self.messages.append({
                    "role": "user",
                    "content": f'''
You are going to test the target website: {target_website}
This is the task you'll try to perform on the target website for the testing purpose. Please interact with the web pages more than 5 times to accomplish the task.
{user_input}

After testing, please generate a testing report that summarizes the whole testing process. The test report should not be more than 100 words.'''
                })
...
if __name__ == "__main__":
    ...
    parser.add_argument('target_website', help="URL of the target website", type=str)
